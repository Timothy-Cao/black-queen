...
//
// Usage: HARD4_TIME_MS=80 npx tsx src/game/_puct_ab.ts 3000 docs/budget_sweep/ucbc_ab.jsonl
...
const wasmNode = await import("./wasm-node/bq_wasm.js") as {
  set_ucb_c_wasm: (c_x100: number) => void;
};
await warmWasm();
...
// c is ucb_c × 100. "c140" = default 1.4 (baseline). Lower c = more exploitation.
const VARIANTS = [
  { name: "c140", c: 140 },  // baseline (default)
  { name: "c070", c: 70 },
  { name: "c100", c: 100 },
  { name: "c200", c: 200 },
];
...
const outPath = argv[3] ?? "docs/budget_sweep/ucbc_ab.jsonl";
mkdirSync(outPath.split("/").slice(0, -1).join("/") || ".", { recursive: true });

const rows: string[] = [];
const t0 = Date.now();
for (let i = 0; i < N; i++) {
  const seed = 7_000_000 + i;
