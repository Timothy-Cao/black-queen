...
//
// Usage: HARD4_TIME_MS=80 npx tsx src/game/_puct_ab.ts 3000 docs/budget_sweep/tree_ab.jsonl
...
const wasmNode = await import("./wasm-node/bq_wasm.js") as {
  set_tree_ismcts_wasm: (enabled: boolean, depth: number) => void;
};
await warmWasm();
...
// Baseline = flat MCTS (tree off). Treatments = tree ISMCTS at various depths.
const VARIANTS = [
  { name: "flat",     tree: false, depth: 0 },
  { name: "tree_d6",  tree: true,  depth: 6 },
  { name: "tree_d10", tree: true,  depth: 10 },
  { name: "tree_d16", tree: true,  depth: 16 },
];
...
function runOne(seed: number, hard4Seat: PlayerId, v: typeof VARIANTS[number]): {
  ...
  wasmNode.set_tree_ismcts_wasm(v.tree, v.depth);
...
const outPath = argv[3] ?? "docs/budget_sweep/tree_ab.jsonl";
mkdirSync(outPath.split("/").slice(0, -1).join("/") || ".", { recursive: true });

const rows: string[] = [];
const t0 = Date.now();
for (let i = 0; i < N; i++) {
  const seed = 8_000_000 + i;
