// Quick A/B: tuned (with void feature) vs tuned (void weights zeroed). Same seeds.
...
const ENABLE = { ...tuned, voidCreateSingletonBonus: 15, voidCreateDoubletonBonus: 5, voidCreateTrumpGate: 1.5 };
const DISABLE = { ...tuned, voidCreateSingletonBonus: 0, voidCreateDoubletonBonus: 0, voidCreateTrumpGate: 1.0 };
...
const N = parseInt(process.argv[2] || "2500", 10);
let enWins = 0, enSeats = 0, disWins = 0, disSeats = 0;
for (let i = 0; i < N; i++) {
  ...
  for (let k = 0; k < 5; k++) if (seats[k] === "hard-2") { enSeats++; if (wA[k]) enWins++; }

  setActiveHardWeights(DISABLE);
  const wB = runOne(seats, 12345 + i * 7919);
  for (let k = 0; k < 5; k++) if (seats[k] === "hard-2") { disSeats++; if (wB[k]) disWins++; }
}
console.log(`Games: ${N} (each pair shares seed)`);
console.log(`ENABLED  win-rate: ${(enWins/enSeats*100).toFixed(2)}%  (${enWins}/${enSeats})`);
console.log(`DISABLED win-rate: ${(disWins/disSeats*100).toFixed(2)}%  (${disWins}/${disSeats})`);
console.log(`Delta: ${((enWins/enSeats - disWins/disSeats)*100).toFixed(2)}pp`);
