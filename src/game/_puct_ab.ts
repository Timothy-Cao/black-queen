...
//
// Usage: HARD4_TIME_MS=80 npx tsx src/game/_puct_ab.ts 3000 docs/budget_sweep/puct_ab.jsonl
...
const wasmNode = await import("./wasm-node/bq_wasm.js") as {
  set_puct_wasm: (enabled: boolean, c_x100: number, conc_x100: number) => void;
};
await warmWasm();
...
// Variants: baseline (PUCT off) + a few prior strengths.
const VARIANTS = [
  { name: "ucb1",          enabled: false, c: 150, conc: 0 },
  { name: "puct_c150_k50", enabled: true,  c: 150, conc: 50 },
  { name: "puct_c150_k35", enabled: true,  c: 150, conc: 35 },
  { name: "puct_c100_k50", enabled: true,  c: 100, conc: 50 },
];
...
function runOne(seed: number, hard4Seat: PlayerId, v: typeof VARIANTS[number]): {
  ...
  wasmNode.set_puct_wasm(v.enabled, v.c, v.conc);
...
const outPath = argv[3] ?? "docs/budget_sweep/puct_ab.jsonl";
