// Rollout-policy A/B harness. Hard-4 in seat 0 vs 4×hard-3.
// Per seed, runs with each rollout policy (Tactical, Greedy, Random).
// Tests sensitivity of ISMCTS to rollout quality — if tactical >> random,
// rollout improvements are a real Hard-5 lever. If similar, ISMCTS is
// rollout-robust and we need a different angle.
//
// Usage: HARD4_TIME_MS=80 npx tsx src/game/_rollout_ab.ts 1000 docs/budget_sweep/rollout_ab.jsonl
...
const wasmNode = await import("./wasm-node/bq_wasm.js") as {
  set_rollout_policy_wasm: (p: number) => void;
};
await warmWasm();
...
const POLICIES = [
  { name: "tactical", code: 0 },
  { name: "greedy",   code: 1 },
  { name: "random",   code: 2 },
];

function runOne(seed: number, hard4Seat: PlayerId, policy: number): {
...
const N = parseInt(argv[2] ?? "1000", 10);
const outPath = argv[3] ?? "docs/budget_sweep/rollout_ab.jsonl";
...
for (let i = 0; i < N; i++) {
  ...
  for (const policy of POLICIES) {
    try {
      const out = runOne(seed, hard4Seat, policy.code);
      rows.push(JSON.stringify({ seed, hard4Seat, policy: policy.name, ...out }));
    } catch (e) {
      rows.push(JSON.stringify({ seed, hard4Seat, policy: policy.name, error: String(e) }));
    }
  }
  if ((i + 1) % 50 === 0) {
...
console.log(`wrote ${rows.length} rows (${N} seeds × ${POLICIES.length} policies) to ${outPath}`);
